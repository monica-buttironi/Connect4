...
function checkEndConditions(state) {
  ...
  winningCandidate = state.filter((p)=>p.color === lastColor);
  return false;
}
...
function main() {
  ...
  function handleInsert() {
    ...
    if(checkEndConditions(state)) return;
    nextX = 0;
    nextZ = 0;
    nextColor = nextColor === DARK ? LIGHT : DARK;
    const {x: new_x, y:  new_y, z: new_z} = convertFromBoardCoordinates(nextX, heightPreview, nextZ);
  ...
  function keyFunctionDown(e) {
    ...
    unloadLastPieceComponent();
    const coordinates = convertFromBoardCoordinates(nextX, heightPreview, nextZ);
    loadSceneComponent(pieceMesh, false, coordinates.x, coordinates.y, coordinates.z);
  }
}
